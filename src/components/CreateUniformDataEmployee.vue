                    ...
                    <div class="flex flex-col bg-white border shadow-sm rounded-xl dark:bg-gray-800 dark:border-gray-700 dark:shadow-slate-700/[.7]">
                        <form @submit.prevent="createUniformEmployee">
                            ...
                            <div class="grid md:grid-cols-2 sm:grid-cols-12 gap-4 p-4 md:p-5">
                                <div>
                                    <label for="hs-validation-name-error" class="block text-sm font-medium mb-2 dark:text-white">Tipos de Uniformes</label>
                                    <select v-model="uniform_type_input" required class="bg-white-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                        <option value="">- Sucursal -</option>
                                        <option v-for="uniform_type in uniform_types" :key="uniform_type.id" :value="uniform_type.id">{{ uniform_type.uniform_type }}</option>
                                    </select>
                                </div>
                                <div>
                                    <label for="hs-validation-name-error" class="block text-sm font-medium mb-2 dark:text-white">Fecha de Despacho</label>
                                    <input
                                        type="date"
...
export default {
    ...
    async created() {
        ...
        try {
            const response = await axios.get('https://apijis.com/uniform_types', {
                headers: {
    ...
    methods: {
        async createUniformEmployee() {
            ...
            const response = await axios.post('https://apijis.com/uniforms/store', dataToSend, {
                headers: {
                ...
                this.$router.push('/uniform_data_employee/' + this.$route.params.id);
            }).catch(error => {
                console.error(error);
                this.loading = false;
            });
