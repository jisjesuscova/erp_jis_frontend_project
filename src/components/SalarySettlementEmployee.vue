...
export default {
    ...
    methods: {
        ...
        async downloadSalarySettlement(id) {
            ...
            try {
                const response = await axios.get('http://localhost:8000/salary_settlements/download/' + id, {
                    headers: {
        ...
        async getSalarySettlements() {
            ...
            try {
                const response = await axios.get('http://localhost:8000/salary_settlements/edit/' + this.$route.params.rut + '/' + page, {
                    headers: {
                    accept: 'application/json',
                    Authorization: `Bearer ${accessToken}` // Agregar el token al encabezado de la solicitud
                    },
