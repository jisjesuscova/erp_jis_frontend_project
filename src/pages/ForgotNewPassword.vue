<template>
    <main class="w-full max-w-md mx-auto p-6">
        ...
        <div
            v-if="verifyPasswords == false"
            class="bg-red-500 text-sm text-white rounded-md p-4 mb-10"
            role="alert"
        >
            Las Contraseñas <span class="font-bold">No</span> Coinciden.
        </div>

        <div
            v-if="forgotNewPassword != 1"
    ...
    methods: {
        ...
        async verifypasswordInput() {
            if (this.password !== this.confirm_password) {
                this.verifyPasswords = false
                return
            }
            else {
                this.verifyPasswords = true
                const accessToken = localStorage.getItem('accessToken')
                const dataToSend = {
                    visual_rut: this.rut,
