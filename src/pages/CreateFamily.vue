...
export default {
    ...
    methods: {
        ...
        validateRut: function(completed_rut) {
            ...
            if (!/^[0-9]+[-|‐]{1}[0-9kK]{1}$/.test(completed_rut))
                return false;
        ...
        createFamilyEmployee() {
            ...
            axios.post('https://apijis.com/family_core_data/store', formData, {
                headers: {
                ...
                this.$router.push('/family_data_employee/' + this.$route.params.id);
            }).catch(error => {
                console.error(error);
                this.loading = false;
            });
