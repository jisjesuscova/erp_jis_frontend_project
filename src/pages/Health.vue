        ...
        <div v-else class="flex flex-col pt-10">
            ...
            <div class="-m-1.5 overflow-x-auto">
                <div
                    ...
                    role="alert"
                    v-if="delete_health == 1"
                >
                     isapre eliminada con <span class="font-bold">éxito</span>.
                </div>
                                    ...
                                    <tr v-for="health in healths" :key="health.id">
                                        ...
                                        >
                                            <router-link
                                                ...
                                                href="javascript:;"
                                                :to="`/edit_health/${health.id}`"
                                            >
                                                <i class="fa-solid fa-pen"></i>
                                            </router-link>
                                            <button
                                                type="button"
                                                @click="confirmHealth(health.id, health.health)"
    ...
    methods: {
        async getHealths() {
            ...
            try {
                ...
                console.error('Error al obtener la lista de Isapres y/o fonasas:', error)
            }
        },
        async confirmHealth(id, health) {
            const shouldDelete = window.confirm(
                `¿Estás seguro de que deseas borrar ${health} ?`
            )
            console.log(id)

            if (shouldDelete) {
                await this.deleteHealth(id)
            }
        },
        async deleteHealth(id) {
            ...
            try {
                const accessToken = localStorage.getItem('accessToken')
                await axios.delete(`http://localhost:8000/healths/delete/${id}`, {
                    headers: {
                        accept: 'application/json',
                        Authorization: `Bearer ${accessToken}`,
                    },
                ...
                this.delete_health = 1
            } catch (error) {
                console.error('Error al borrar la isapre o fonasa seleccionada:', error)
            }
        },
    },
    async mounted() {
